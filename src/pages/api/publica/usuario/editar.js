...
export default async function apiPublicaUsuarioEditar(req, res) {
  let method = 'PUT';
  ...
  if (req?.method === method) {
    try {
      ...
      let resBancoDeDados = await Usuario.findOneAndUpdate({ _id: condicoes._id }, dados, { runValidators: true });
      if(resBancoDeDados){
